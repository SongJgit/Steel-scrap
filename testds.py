...
class StartCV():
    ...
    def __init__(self, num_camera=3,path='image'):
        ...
        """for i in range(self.num_camera):
            setattr(self,'img_'+str(i),'self.camera_{}.read()[1]'.format(i))"""
    ...
    def open(self):
        ...
        while 1:
            ...
            """funcname = 'self.camera_{}'.format(i)
                createVar['ret_'+str(i),'self.img_'+str(i)] = globals()[funcname].read()"""
            if self.num_camera == 1:
                funcname = 'self.camera_{}'.format(0)
                createVar['ret_'+str(0),'self.img_'+str(0)] = globals()[funcname].read()
                #self.img_0=self.camera_{}.read()[1]
