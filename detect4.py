...
def GetRegion(img):
    ...
    for contour in contours:
        ...
        if (area < 2000):
            continue
        eps = 1e-3 * cv2.arcLength(contour, True)  # 轮廓周长
        approx = cv2.approxPolyDP(contour, eps, True)  # 获得轮廓的近似
        rect = cv2.minAreaRect(contour) # 求出参数中的最小面积的矩形
...
if __name__ == '__main__':
  #输入的参数为图片的路径
    img = cv2.imread('image/20201228/12345.jpg')
    detect(img)